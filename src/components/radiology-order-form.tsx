...
import { RADIOLOGY_TESTS, type RadiologyTest, type RadiologyTestCategory } from '@/lib/radiology-tests';
import { useDebounce } from '@/hooks/use-debounce';
import { Monitor } from 'lucide-react';

interface RadiologyOrderFormProps {
  onSubmitted: (selectedStudies: string[]) => void;
  onCancel: () => void;
}

export function RadiologyOrderForm({ onSubmitted, onCancel }: RadiologyOrderFormProps) {
  ...
  const filteredCategories = React.useMemo(() => {
    if (!debouncedSearchTerm) {
      return RADIOLOGY_TESTS;
    }
    const lowercasedFilter = debouncedSearchTerm.toLowerCase();
    const filtered: RadiologyTestCategory[] = [];

    RADIOLOGY_TESTS.forEach(category => {
  ...
  const handleSubmit = () => {
    const selected = Object.entries(selectedTests)
      ...
      .map(([testId]) => {
          for (const category of RADIOLOGY_TESTS) {
  ...
  return (
    <div className="flex flex-col h-[70vh]">
      <div className="p-4 border-b">
        <Input
          placeholder="Buscar estudio de imagenología..."
          value={searchTerm}
      ...
      <div className="flex justify-between items-center p-4 border-t bg-background">
        <div className="text-sm text-muted-foreground">
          <span className="font-bold">{selectedCount}</span> estudio(s) seleccionado(s)
        </div>
